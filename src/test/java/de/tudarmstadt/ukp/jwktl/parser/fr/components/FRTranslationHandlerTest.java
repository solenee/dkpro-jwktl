...
/**
 * Test case for {@link FRTranslationHandler}.
 * @author Christian M. Meyer
 */
public class FRTranslationHandlerTest extends FRWiktionaryEntryParserTest {

	public void testHandleT() throws Exception {
		IWiktionaryTranslation translation = processFirst("* {{T|en}} : {{trad+|en|book}}");
		...
		translation = processFirst("* {{T|de}} : {{trad-|de|Buch}}");
...
*/
	...
	public void testManga() throws Exception {
		...
		assertTranslation("English", "manga", "Bande dessinée japonaise", iter.next());
		assertTranslation("Korean", "만화", "Bande dessinée japonaise", iter.next());
		assertTranslation("Esperanto", "mangao", "Bande dessinée japonaise", iter.next());
		assertTranslation("Japanese", "漫画", "Bande dessinée japonaise", iter.next());
		assertTranslation("Swedish", "manga", "Bande dessinée japonaise", iter.next());
		assertFalse(iter.hasNext());
	}

	/** 
	 * Test that the api handle the following pattern: {{T|en}} : {{trad+|en|remission}} (1, 2, 3, 4, 5)
	 * @throws Exception
	 */
	public void testHandleBrackets() throws Exception {
		IWiktionaryPage page = parse("rémission.txt");
		Iterator<IWiktionaryTranslation> iter = page.getEntry(0).getUnassignedSense().getTranslations().iterator();
		assertTranslation("English", "remission", "", iter.next());
		assertTranslation("io", "remiso", "", iter.next());
		assertTranslation("id", "ampun", "", iter.next());
		assertTranslation("it", "remissione", "", iter.next());
		assertTranslation("nl", "remissie", "", iter.next());
		//* {{T|pl}} : {{trad+|pl|przebaczenie|n}}, {{trad+|pl|odpuszczenie|n}}
		assertTranslation("pl", "przebaczenie", "", iter.next());
		assertTranslation("pl", "odpuszczenie", "", iter.next()); 		
		assertFalse(iter.hasNext());
	}
	/***/
...
*/
	...
	protected List<IWiktionaryTranslation> process(String... body) {
		...
		ParsingContext context = new ParsingContext(new WiktionaryPage(), new FREntryFactory() {
			@Override
			public WiktionaryEntry findEntry(ParsingContext context) {
				return entry;
			}
		});
		FRTranslationHandler handler = new FRTranslationHandler();
