 ...
 * @author Lizhen Qu
 */
public class FRRelationHandler extends FRBlockHandler {
	/**
	 * Initializes the block handler for the given relation type and
	 * section headers.
	 */
	public FRRelationHandler(final RelationType relationType, final String... labels) {
		super(labels);
	...
	public boolean processHead(String textLine, ParsingContext context) {
		...
		//System.err.println("[FRRelationHandler.processHead]");// SEH
...
//			System.err.println("[processBody]"+line);// SEH
			relationList.add(parseWordList(line.substring(1)));
//			System.err.println("[processBody]"+relationList.size());// SEH
//			for (WordList wl : relationList) {
//				System.err.println(wl.words.size()+" "+wl.words);// SEH
//			}
		}
		return false;
	}
